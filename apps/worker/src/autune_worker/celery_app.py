...
celery_app.conf.update(
    ...
    task_routes={
        ...
        # Only the Slack calls -- no embedder/reranker/NLI model work -- so it
        # does not belong on the worker reserved for that. notify_late_drift is
        # the same shape (see autune_context.tasks._slack_target, shared by
        # both) and needs the same override.
