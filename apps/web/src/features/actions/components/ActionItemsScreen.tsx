...
export function ActionItemsScreen({ meetingId }: { meetingId: string }) {
  const { items, loading, error, add, edit, remove } = useActionItems({
  ...
  return (
    <main
      className="mx-auto flex max-w-[1200px] gap-6"
        ...
        <div className="mt-6">
          {items.length > 0 || (!loading && error === null) ? (
            <>
              {error ? (
                <Note>
                  최신 목록을 불러오지 못해 이전 목록을 보여주고 있습니다.
                </Note>
              ) : null}
              {items.length === 0 ? (
                <Note>
                  이 회의에서 추출된 액션 아이템이 없습니다. 놓친 항목은 직접
                  추가할 수 있습니다.
                </Note>
              ) : null}
              <ActionBoard
