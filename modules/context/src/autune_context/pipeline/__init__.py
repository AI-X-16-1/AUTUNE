...
warms and logs each model so the first task does not pay the load cost and a
misconfigured or unreachable endpoint fails at startup, not mid-meeting (the
``*Http`` clients probe ``/health`` in their constructor).
