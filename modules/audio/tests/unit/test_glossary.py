...
class TestOrdering:
    ...
    def test_hotwords_puts_the_most_important_first(self) -> None:
        ...
        First among the terms, that is — the framing sentence comes before them
        and is ten tokens the measurement says are worth spending.
        """
        prompt = build_prompt(participants=["박준호"], mode="hotwords")
        assert prompt.split(": ", 1)[1].startswith("박준호")
    ...
    def test_both_follows_hotwords(self) -> None:
        ...
        prompt = build_prompt(participants=["박준호"], mode="both")
        assert prompt.split(": ", 1)[1].startswith("박준호")
...
class TestAssembly:
    ...
    def test_both_channels_get_the_framing_sentence(self) -> None:
        """Not obvious for ``hotwords``, and measured rather than assumed.

        The prefix costs ten tokens and boosts 회의 and 녹취록 along with the
        terms, so it was removed. Term accuracy on the evaluation recording came
        out 45% without it against 62% with it, same terms and same order:
        ``hotwords`` is read as previous text, and a bare list of English words
        is not what a transcript looks like.
        """
        assert build_prompt(mode="prompt").startswith("회의 녹취록")
        assert build_prompt(mode="hotwords").startswith("회의 녹취록")
