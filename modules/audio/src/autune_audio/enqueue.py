...
def enqueue_process_recording(job_id: str) -> None:
    ...
    **The message carries the job id and nothing else.** Celery puts task
    arguments in the broker message and in its own failure output, so anything
    passed here is a thing that travels; privacy.md section 1 forbids a path to
    raw audio in a Celery payload, and the worker rebuilds the path from the id
    (``storage.upload_path``) rather than being told it (#275).
    """
    current_app.send_task(PROCESS_RECORDING, args=[job_id])
    log.info("audio_process_queued", job_id=job_id)
